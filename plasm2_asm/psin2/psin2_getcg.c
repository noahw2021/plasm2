...
BYTE Psin2iGetPhysicalSize(int Id, BYTE OperandId) {
	return
        Psin2Ctx->Instructions[Id].Operands[OperandId].
        PhysicalSize;
}

BYTE Psin2iGetAvailableSize(int Id, BYTE OperandId) {
	return Psin2Ctx->Instructions[Id].Operands[OperandId].
        AvailableSize;
}

char* Psin2iGetOperandName(int Id, BYTE OperandId) {
	char* ret = Psin2Ctx->Instructions[Id].Operands[OperandId].
        OperandName;
...
int Psin2iGetInstructionByName(const char* Operand) {
	for (int i = 0; i < Psin2Ctx->InstructionCount; i++) {
		if (Psin2Ctx->Instructions[i].Instruction[0] == 
            Operand[0] && Psin2Ctx->Instructions[i].
